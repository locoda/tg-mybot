        ...
        reply_markup: {
          inline_keyboard: [
            [
              {
                text: "请加入日剧私藏频道",
                url: "https://t.me/+Cr7NSig2jc00NWZh",
              },
            ],
            [
              {
                text: "完整版（Telegraph ver.）",
...
function handleJplsCallback(callback_query) {
  ...
  if (Array.isArray(list)) {
    list = list.map((item) => parseJptvItemMd(item));
    var text = "获取到结果：\n" + list.join("\n");
    var buttons = [];
  } else {
    var text = "请继续选择操作⬇️";
...
function handleJptvCallback(callback_query) {
  ...
  var result = scriptProperties.getProperty(
    "jptv:" + callback_query.message.chat.id
  );
  ...
  editMessageText({
    ...
    text:
      "搜索结果（" +
      (Math.ceil(index / 10) + 1) +
      "/" +
      (Math.floor((result.length - 1) / 10) + 1) +
      "）： \n" +
